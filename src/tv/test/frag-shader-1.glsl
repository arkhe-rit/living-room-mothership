...
void main() {
    ...
    float vertMovementOn = (1.0 - step(snoise(vec2(u_time * 0.2, 8.0)), 0.4));
    float vertJerk = (1.0 - step(snoise(vec2(u_time * 1.5, 5.0)), 0.6));
    float vertJerk2 = (1.0 - step(snoise(vec2(u_time * 5.5, 5.0)), 0.2));
    float yOffset = abs(sin(u_time) * 4.0) * vertMovementOn + vertJerk * vertJerk2 * 0.3;
    float y = mod(uv.y + yOffset, 1.0);
    
    float xOffset = (fuzzOffset + largeFuzzOffset) * 1.0;
    ...
    vec3 color = texture2D(u_texture, vec2(uv.x + xOffset, y)).rgb;
    color += staticVal;
