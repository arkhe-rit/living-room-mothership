...
float staticV(vec2 uv) {
  float staticHeight = snoise(vec2(9.0,u_time*1.2+3.0))*u_staticBounce+5.0;
...
void main() {
    ...
    float vertJerk2 = (1.0 - step(snoise(vec2(u_time * 5.5, 5.0)), 0.2)) * u_vertJerkOpt;
    ...
    for (float dy = -1.0; dy <= 1.0; dy += 1.0) {
        ...
        float dist = dy / 100.0;
        staticVal += staticV(vec2(uv.x, uv.y + dist)) * (maxDist - abs(dist)) * u_staticStr;
    }
